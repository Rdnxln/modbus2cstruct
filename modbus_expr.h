...
/* Перечисление поддерживаемых бинарных операций */
typedef enum {
  /*  *       /       %       +       - */
...
typedef enum {
  UNARY_NOT,  /* !A Логическая инверсия */
  UNARY_BNOT, /* ~A Битовое отрицание  */
  UNARY_NEG   /* -A Смена знака */
} unop_t;
...
/* описание поля целевой структуры,
   куда сохраняется результат */
typedef struct {
  const char   *name;      /* имя */
  size_t        offset;    /* смещение от начала структуры */
  field_type_t  type;      /* тип данных языка C */
